package be.mve.tsm.config.timezone;

import be.mve.tsm.TsmApp;
import be.mve.tsm.repository.timezone.DateTimeWrapper;
import be.mve.tsm.repository.timezone.DateTimeWrapperRepository;
...
@SpringBootTest(classes = TsmApp.class)
public class HibernateTimeZoneTest {
    ...
    private String generateSqlRequest(String fieldName, long id) {
        return format("SELECT %s FROM tsmjhi_date_time_wrapper where id=%d", fieldName, id);
    }
